...
function GetData(){
    ...
    wget -O database.zip -q --user-agent="Docker-IP2Location/MySQL" http://www.ip2location.com/download?token=${TOKEN}\&productcode=${CODE_IMPORTED} > /dev/null 2>&1
...
function ImportData(){
    ...
    if [ "$TABLE_TO_IMPORT" == "DB1" ]; then
        CSV="$(find . -name 'IPCountry.csv')"

    elif [ "$TABLE_TO_IMPORT" == "DB2" ]; then
        CSV="$(find . -name 'IPISP.csv')"

    elif [ ! -z "$(echo $TABLE_TO_IMPORT | grep 'LITE')" ]; then
        CSV="$(find . -name 'IP*.CSV')"

    elif [ ! -z "$(echo $TABLE_TO_IMPORT | grep 'IPV6')" ]; then
...
function RenameTable(){
    ...
    if [[ "$TABLE_TO_IMPORT" == *IPV6 ]]; then
        TABLE_NAME="table_ipv6"
    else
        TABLE_NAME="table_ipv4"
    fi


    echo -n " > [MySQL] Rename table \"ip2location_database_tmp\" to \"$TABLE_NAME\" "

    RESPONSE="$(mysql ip2location_database -e 'RENAME TABLE ip2location_database_tmp TO '$TABLE_NAME'' 2>&1)"
